...
//    IN ANY WAY WITHOUT PRIOR WRITTEN CONSENT FROM POWERSOFT CORP.
//
// ------------------------------------------------------------------------
//
//    Filename :        pbarray.h
...
{
	typedef PBValueTraits<pbvalue_int>::ValueType ValueType;
...
//	if (!d_array || !d_value)
//		throw PBXHelperException(_TEXT("create PBUnboundedArrayCreator failed"));
}
...
//	if (!d_array || !d_value)
//		throw PBXHelperException(_TEXT("create PBBoundedArrayCreator failed"));
}
...
template <pbvalue_type T>
PBArrayAccessor<pbvalue_int>::ValueType	PBArrayAccessor<T>::GetAt(pblong dim[], pbboolean& isNull)
{
