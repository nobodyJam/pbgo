#include "pbrt.h"
#include "D:\\Sybase\\PowerBuilder 9.0\\SDK\\PBNI\\include\\pbni.h"
...
char* test(){
	...
	if ( hinst== NULL)   return "Error in CreateSession\n";   
    printf("Loaded PBVM successfully\n");
	P_PB_GetVM getvm = (P_PB_GetVM)GetProcAddress      (hinst,"PB_GetVM");  
	if (getvm == NULL) return "Error in CreateSession\n";    
	getvm(&pbvm);  
	if (pbvm == NULL) return "Error in CreateSession\n";
